...
| **`interval`**               | <code>number</code>  | Interval in milliseconds to receive location updates in `watchPosition`. For backwards compatiblity with version 7.1.x, if no value is passed, the default value of this parameter is that of `timeout`. If you are experiencing location timeouts with watch position, try setting `interval` to a value lower than `timeout`. This parameter is only available for Android. It has no effect on iOS or Web platforms.                                                                                                                                                                                                                                                                                                                             | <code>`timeout`</code> | 8.0.0 |
